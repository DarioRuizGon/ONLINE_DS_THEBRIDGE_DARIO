...
def transform_and_scale(train, cols, target, test = None, lists = False, value_for_log_box = 1, try_log_box = True):
    train_scaled = train.copy()
    if test: 
        test_scaled = test.copy()    

    power_boxcox = PowerTransformer("box-cox")
    power_yeo = PowerTransformer("yeo-johnson")

    # Inicializo las listas para guardar cada variable y hacerle la transformación que le corresponde
    boxcox_list = []
    yeo_list = []
    log_list = []
    sqrt_list = []
    no_transformation_list = []

    for col in cols:
        dict_shapiro = {}

        # Compruebo las 3 veriones que se pueden obtener con valores 0 y negativos
        dict_shapiro[shapiro(train_scaled[col]).pvalue] = no_transformation_list
        dict_shapiro[shapiro(power_yeo.fit_transform(train_scaled[[col]])).pvalue] = yeo_list
        dict_shapiro[shapiro(np.sqrt(train_scaled[col])).pvalue] = sqrt_list

        # Para las variables que tienen valores 0 y negativos sumo el valor elegido, como he trabajado con ellas, sé que no hay ninguna que tenga un mínimo que llegue a -1
        if train_scaled[col].min() <= 0:
            if try_log_box:
                dict_shapiro[shapiro(power_boxcox.fit_transform(train_scaled[[col]] + value_for_log_box)).pvalue] = boxcox_list
                dict_shapiro[shapiro(np.log(train_scaled[col] + value_for_log_box)).pvalue] = log_list

        else:
            dict_shapiro[shapiro(power_boxcox.fit_transform(train_scaled[[col]])).pvalue] = boxcox_list
            dict_shapiro[shapiro(np.log(train_scaled[col])).pvalue] = log_list

        
        
        if train_scaled[col].min() <= 0 and (dict_shapiro[max(dict_shapiro.keys())] == boxcox_list or dict_shapiro[max(dict_shapiro.keys())] == log_list):
            train_scaled[col] = train_scaled[col] + value_for_log_box # Si la variable tiene valores 0 o negativos y le voy a aplicar una transformación que no los admite, le sumo el valor elegido en el dataset
            if test:
                test_scaled[col] = test_scaled[col] + value_for_log_box
            dict_shapiro[max(dict_shapiro.keys())].append(col)
    ...
    [("Box-cox", PowerTransformer("box-cox", standardize= True), boxcox_list), # Le hago a cada variable la transformación más adecuada y las dejo escaladas
     ("Yeo-Johnson", PowerTransformer("yeo-johnson", standardize= True), yeo_list)],
     remainder= "passthrough"
    )

    gaussian_transformer.fit(train_scaled)
    train_scaled = pd.DataFrame(gaussian_transformer.fit_transform(train_scaled), columns= [re.match(r".*__(.*)", col).group(1) for col in gaussian_transformer.get_feature_names_out()], index= train_scaled.index)
    
    train_scaled[sqrt_list] = np.sqrt(train_scaled[sqrt_list]) # Las transformaciones que faltan
    train_scaled[log_list] = np.log(train_scaled[log_list])

    scaler = ColumnTransformer(
    [("Std_scaler", StandardScaler(), [col for col in train_scaled if col not in boxcox_list + yeo_list + [target]])],
    remainder= "passthrough"
    )

    scaler.fit(train_scaled)
    train_scaled = pd.DataFrame(scaler.transform(train_scaled), columns= [re.match(r".*__(.*)", col).group(1) for col in scaler.get_feature_names_out()], index= train_scaled.index)
    
    if test and not lists:
        test_scaled = pd.DataFrame(gaussian_transformer.transform(test_scaled), columns= [re.match(r".*__(.*)", col).group(1) for col in gaussian_transformer.get_feature_names_out()], index= test_scaled.index)
        test_scaled[sqrt_list] = np.sqrt(test_scaled[sqrt_list]) # Las transformaciones que faltan
        test_scaled[log_list] = np.log(test_scaled[log_list])
        test_scaled = pd.DataFrame(scaler.transform(test_scaled), columns= [re.match(r".*__(.*)", col).group(1) for col in scaler.get_feature_names_out()], index= test_scaled.index)

        return (train_scaled, test_scaled)
    
    elif lists and not test:
        return([boxcox_list, yeo_list, log_list, sqrt_list], train)
    
    elif test and lists:
        return([boxcox_list, yeo_list, log_list, sqrt_list], train, test)
